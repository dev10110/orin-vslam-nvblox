  ...
  VslamPX4Bridge() : Node("vslam_px4_bridge") {
    ...
    position_sub_ =
        this->create_subscription<nav_msgs::msg::Odometry>(
            vslam_sub_name, 1,
            std::bind(&VslamPX4Bridge::callback, this, _1));

    odometry_pub_ = this->create_publisher<px4_msgs::msg::VehicleOdometry>(
        px4_pub_name, 10);

    RCLCPP_INFO(get_logger(), "Subscribed to %s", position_sub_->get_topic_name());
    RCLCPP_INFO(get_logger(), "Publishing to %s", odometry_pub_->get_topic_name());
  }
...
private:
  ...
  void callback(const nav_msgs::msg::Odometry &vslam_msg) {
    ...
    // get the timestamp
    uint64_t sample_microseconds = vslam_msg.header.stamp.sec * 1e6 + vslam_msg.header.stamp.nanosec * 1e3;
    ...
    Eigen::Vector3d p_enu(
		    vslam_msg.pose.pose.position.x,
		    vslam_msg.pose.pose.position.y,
		    vslam_msg.pose.pose.position.z);


    Eigen::Quaterniond q_enu(
		    vslam_msg.pose.pose.orientation.w,
		    vslam_msg.pose.pose.orientation.x,
		    vslam_msg.pose.pose.orientation.y,
		    vslam_msg.pose.pose.orientation.z);
    ...
    if ((p_enu - last_pos).norm() > 0.01)
    {
	    // MOVED TOO MUCH! 
	    // EXIT
	    RCLCPP_ERROR(get_logger(), "VSLAM JUMPED by %f m. Exiting...", (p_enu - last_pos).norm());
	    rclcpp::shutdown();
    }
    ...
    // // transform to NED
    // Eigen::Vector3d p_ned = transform_static_frame(p_enu, StaticTF::ENU_TO_NED);
    // Eigen::Quaterniond q_ned = transform_orientation(q_enu, StaticTF::ENU_TO_NED);
    
    RCLCPP_INFO(get_logger(), "ENU POS: %f, %f, %f", p_enu[0], p_enu[1], p_enu[2]);
    RCLCPP_INFO(get_logger(), "NED POS: %f, %f, %f", p_ned[0], p_ned[1], p_ned[2]);
