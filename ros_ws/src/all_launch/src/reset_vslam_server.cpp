...
Eigen::Affine3d transformToEigen(const geometry_msgs::msg::Transform& transform) {
    ...
    Eigen::Quaterniond quaternion(transform.rotation.w, transform.rotation.x, transform.rotation.y, transform.rotation.z);
...
geometry_msgs::msg::Transform eigenToTransform(const Eigen::Affine3d& eigen_transform) {
...
private:
    ...
    bool get_transform(geometry_msgs::msg::TransformStamped & transform, const std::string & header_frame, const std::string child_frame)
    {
        try {
            // Lookup the current transform
            transform = tf_buffer_->lookupTransform(header_frame, child_frame, rclcpp::Time(0));
	    return true;
    ...
    void set_transform_callback(const std::shared_ptr<vslam_interfaces::srv::ResetVslam::Request> request,
	...
	bool success_1 = get_transform(transform_vicon, "vicon/world", "vicon/px4_2/px4_2");
